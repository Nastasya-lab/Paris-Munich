...
LONGITUDE = -3.5608
NEIGHBORS = ["LETO"]
ADVECTION_STATIONS = ["LEMD", "LETO"]
ISSUE_HOURS = [6, 8, 10, 12, 14, 16, 18, 20]
MODEL_VERSION = "lemd_metar_tmax_multinwp_d1_v1"
BASE_FEATURES = list(dict.fromkeys(DEFAULT_METAR_TMAX_FEATURES + list(ENHANCED_METAR_INTRADAY_FEATURES)))
PREFIXES = list(LEMD_NWP_MODELS.values())
AGGREGATE_FEATURES = [
...
def _prepare_nwp(args: argparse.Namespace) -> None:
    ...
    for model, prefix in LEMD_NWP_MODELS.items():
        ...
        hourly_path = cache / f"lemd_{model}_previous_day1_hourly.parquet"
        ...
        snapshot_path = cache / f"lemd_{model}_previous_day1_snapshots.parquet"
    ...
    Path(args.report_dir, "lemd_multinwp_availability.json").write_text(
...
def _build_dataset(args: argparse.Namespace) -> pd.DataFrame:
    ...
    metar = pd.read_parquet(metar_dir / "metar_iem_LEMD.parquet")
    target = build_daily_metar_tmax(
        metar,
        airport_icao=AIRPORT,
        timezone_name=TIMEZONE,
        source_id="iem.metar.archive.LEMD",
    ...
    for model, prefix in LEMD_NWP_MODELS.items():
        nwp = pd.read_parquet(Path(args.nwp_cache_dir) / f"lemd_{model}_previous_day1_snapshots.parquet")
    ...
    neighbor_metars = {station: pd.read_parquet(metar_dir / f"metar_iem_{station}.parquet") for station in NEIGHBORS}
    ...
    Path(args.report_dir).mkdir(parents=True, exist_ok=True)
    Path(args.report_dir, "lemd_multinwp_dataset.json").write_text(
        json.dumps(report, indent=2, default=str), encoding="utf-8"
    )
...
def _backtest(args: argparse.Namespace) -> dict:
    ...
    report = {
        "airport": AIRPORT,
        "experiment": "LEMD leakage-safe D-1 ICON-EU/ECMWF/GFS/ARPEGE METAR Tmax",
    ...
    scored.to_parquet(report_dir / "lemd_multinwp_backtest_rows.parquet", index=False)
    summary.to_csv(report_dir / "lemd_multinwp_backtest_summary.csv", index=False)
    daytime.to_csv(report_dir / "lemd_multinwp_backtest_10_17_summary.csv", index=False)
    by_hour.to_csv(report_dir / "lemd_multinwp_backtest_by_hour.csv", index=False)
    by_season.to_csv(report_dir / "lemd_multinwp_backtest_by_season.csv", index=False)
    (report_dir / "lemd_multinwp_backtest.json").write_text(
        json.dumps(report, indent=2, default=str), encoding="utf-8"
    )
...
def _train(args: argparse.Namespace, selected_variant: str | None = None) -> dict:
    report_path = Path(args.report_dir) / "lemd_multinwp_backtest.json"
    ...
    metadata = {
        **artifact.to_metadata(),
        ...
        "target": "maximum integer temperature reported by LEMD METAR during the local day",
        "training_source": "IEM METAR archive plus Open-Meteo Previous Runs day1",
        "nwp_models": LEMD_NWP_MODELS,
        ...
        "limitations": [
            ...
            "Only LETO passed the initial nearby-METAR completeness audit.",
...
def _common_args(parser):
    ...
    parser.add_argument("--metar-dir", default="data/interim/lemd_multinwp")
    parser.add_argument("--nwp-cache-dir", default="data/forecasts/lemd_multinwp")
    parser.add_argument("--target-path", default="data/processed/metar_tmax_target_LEMD.parquet")
    parser.add_argument("--dataset-path", default="data/processed/metar_upside_dataset_LEMD_multinwp.parquet")
    parser.add_argument("--report-dir", default="data/reports/lemd_multinwp")
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Build, backtest and train the LEMD multi-NWP METAR Tmax model.")
