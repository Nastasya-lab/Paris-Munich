...
MODEL_VERSION = "lfpb_metar_tmax_icon_d2_spatial_wind_advection_v1"
        ...
        "data_snapshot_hash": stable_hash(
            {
                ...
                "advection_available_sum": float(frame["adv_available_station_count"].sum()),
            }
        ),
        "created_at_utc": datetime.now(UTC).isoformat(),
        "promotion_status": "replaces_spatial_candidate_block",
        "limitations": [
            "Replaces the previous LFPG/LFPO spatial candidate block in LFPB Telegram reports.",
            "Still reported as a parallel candidate; main production forecast remains lfpb_metar_tmax_icon_d2_v1.",
            "Offline backtest improved expected-value MAE over spatial-only but probabilistic NLL needs monitoring.",
        ],
    }
...
def _load_or_build_dataset(args: argparse.Namespace) -> pd.DataFrame:
    ...
    if not set(spatial_feature_columns(args.neighbor_station)).issubset(base.columns):
        ...
        base = add_spatial_metar_features_to_frame(base, neighbors, timezone_name=args.timezone, stations=args.neighbor_station)
    station_metars = {
        station: pd.read_parquet(Path(args.neighbor_dir) / f"metar_iem_{station}.parquet")
...
def _score_holdout(test: pd.DataFrame, model: IconD2MetarTmaxEnsemble) -> pd.DataFrame:
    ...
    for _, row in test.iterrows():
        ...
        rows.append(
            {
                "model_variant": "spatial_wind_advection_icon_d2",
                "target_date_local": str(row["target_date_local"]),
                "issue_time_utc": pd.Timestamp(row["issue_time_utc"]).isoformat(),
                "local_issue_hour": int(row["local_issue_hour"]),
