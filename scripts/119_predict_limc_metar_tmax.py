...
LATITUDE = 45.6306
LONGITUDE = 8.7281
MODEL_PATH = Path("data/models/limc_metar_tmax_icon_d2_d1_v1.joblib")
METADATA_PATH = Path("data/models/limc_metar_tmax_icon_d2_d1_v1.metadata.json")
METAR_PATH = Path("data/forecasts/awc_metar_live_LIMC.parquet")
REPORT_PATH = Path("data/reports/latest_limc_prediction.json")
HISTORY_PATH = Path("data/logs/limc_forecast_history.jsonl")
LIVE_CACHE_DIR = Path("data/cache/limc_previous_day1_live")
...
def main() -> None:
    ...
    if issue.astimezone(ZoneInfo(TIMEZONE)).date() != target:
        raise ValueError("LIMC live forecast currently supports the current local day only")
    ...
    if not source_status["usable"]:
        raise RuntimeError(
            f"LIMC forecast requires at least {source_status['minimum_required']} NWP sources; "
...
def load_live_nwp_features(
    ...
    *,
    ...
    errors: dict[str, str] = {}
    with ThreadPoolExecutor(max_workers=len(LIMC_NWP_MODELS)) as pool:
        futures = {
    ...
    features = {}
    for prefix in LIMC_NWP_MODELS.values():
...
def format_forecast_message(payload: dict) -> str:
    ...
    lines = [
        "<b>LIMC Milan Tmax forecast</b>",
        f"Дата прогноза: <b>{html.escape(payload['target_date_local'])}</b>",
        f"Выпуск: <b>{local_issue} по Милану</b>",
        "",
    ...
    lines.append("Production source: ICON-D2")
    labels = {"icon_d2": "ICON-D2", "icon_eu": "ICON-EU", "arpege": "ARPEGE Europe"}
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a live LIMC multi-NWP METAR Tmax forecast.")
