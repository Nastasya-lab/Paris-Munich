...
STATE_PATH = Path("data/logs/rcss_metar_event_state.json")
METAR_PATH = Path("data/forecasts/awc_metar_live_RCSS.parquet")


def main() -> None:
    args = _parse_args()
    refresh = refresh_awc_live("RCSS")
    ...
    if latest is not None and latest != previous:
        ...
        _save_state(latest)
        print(json.dumps({"status": "new_metar_forecast", "airport": "RCSS", "latest_metar_time_utc": latest, "refresh": refresh}, indent=2))
        return
    print(json.dumps({"status": "no_new_metar", "airport": "RCSS", "latest_metar_time_utc": latest, "previous_metar_time_utc": previous, "refresh": refresh}, indent=2))
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Poll RCSS METAR and forecast on a new report.")
