...
from weather_tmax_bot.features.spatial_metar import spatial_feature_columns
from weather_tmax_bot.features.wind_advection import wind_advection_feature_columns
...
def _prepare_nwp(args: argparse.Namespace) -> None:
    ...
    availability = []
    for model, prefix in LIMC_NWP_MODELS.items():
        ...
        hourly_path = cache / f"limc_{model}_previous_day1_hourly.parquet"
        ...
        snapshot_path = cache / f"limc_{model}_previous_day1_snapshots.parquet"
    ...
    Path(args.report_dir, "limc_multinwp_availability.json").write_text(
...
def _build_dataset(args: argparse.Namespace) -> pd.DataFrame:
    ...
    metar = pd.read_parquet(metar_dir / "metar_iem_LIMC.parquet")
    target = build_daily_metar_tmax(
        metar,
        airport_icao=AIRPORT,
        timezone_name=TIMEZONE,
        source_id="iem.metar.archive.LIMC",
    ...
    for model, prefix in LIMC_NWP_MODELS.items():
        nwp = pd.read_parquet(Path(args.nwp_cache_dir) / f"limc_{model}_previous_day1_snapshots.parquet")
    ...
    Path(args.report_dir).mkdir(parents=True, exist_ok=True)
    Path(args.report_dir, "limc_multinwp_dataset.json").write_text(
        json.dumps(report, indent=2, default=str), encoding="utf-8"
    )
...
def _backtest(args: argparse.Namespace) -> dict:
    ...
    report = {
        "airport": AIRPORT,
        "experiment": "LIMC leakage-safe D-1 ICON-D2/ICON-EU/ARPEGE METAR Tmax",
    ...
    scored.to_parquet(report_dir / "limc_multinwp_backtest_rows.parquet", index=False)
    summary.to_csv(report_dir / "limc_multinwp_backtest_summary.csv", index=False)
    daytime.to_csv(report_dir / "limc_multinwp_backtest_10_17_summary.csv", index=False)
    by_hour.to_csv(report_dir / "limc_multinwp_backtest_by_hour.csv", index=False)
    by_season.to_csv(report_dir / "limc_multinwp_backtest_by_season.csv", index=False)
    (report_dir / "limc_multinwp_backtest.json").write_text(
        json.dumps(report, indent=2, default=str), encoding="utf-8"
    )
...
def _train(args: argparse.Namespace, selected_variant: str | None = None) -> dict:
    report_path = Path(args.report_dir) / "limc_multinwp_backtest.json"
    ...
    artifact = MultiNwpMetarTmaxModel(
        ...
        minimum_nwp_models=1 if prefix else 2,
    ...
    metadata = {
        **artifact.to_metadata(),
        ...
        "target": "maximum integer temperature reported by LIMC METAR during the local day",
        "training_source": "IEM METAR archive plus Open-Meteo Previous Runs day1",
        "nwp_models": LIMC_NWP_MODELS,
        ...
        "limitations": [
            ...
            "No neighboring METAR station is used in the selected base experiment.",
            (
                f"Live prediction requires {prefix} when the selected single-provider model is active."
                if prefix
                else "Live prediction requires at least two of three NWP providers."
            ),
        ],
    }
...
def _select_variant(summary: pd.DataFrame, daytime: pd.DataFrame) -> dict:
    candidates = summary[summary["model_variant"].str.startswith("metar_")].copy()
...
def _common_args(parser):
    ...
    parser.add_argument("--metar-dir", default="data/interim/limc_multinwp")
    parser.add_argument("--nwp-cache-dir", default="data/forecasts/limc_multinwp")
    parser.add_argument("--target-path", default="data/processed/metar_tmax_target_LIMC.parquet")
    parser.add_argument("--dataset-path", default="data/processed/metar_upside_dataset_LIMC_multinwp.parquet")
    parser.add_argument("--report-dir", default="data/reports/limc_multinwp")
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Build, backtest and train the LIMC multi-NWP METAR Tmax model.")
