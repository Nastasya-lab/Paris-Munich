...
STATE_PATH = Path("data/logs/limc_metar_event_state.json")
METAR_PATH = Path("data/forecasts/awc_metar_live_LIMC.parquet")


def main() -> None:
    args = _parse_args()
    refresh = refresh_awc_live("LIMC")
    ...
    if latest is not None and latest != previous:
        ...
        _save_state(latest)
        print(json.dumps({"status": "new_metar_forecast", "airport": "LIMC", "latest_metar_time_utc": latest, "refresh": refresh}, indent=2))
        return
    print(json.dumps({"status": "no_new_metar", "airport": "LIMC", "latest_metar_time_utc": latest, "previous_metar_time_utc": previous, "refresh": refresh}, indent=2))
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Poll LIMC METAR and forecast on a new report.")
