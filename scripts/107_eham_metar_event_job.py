...
STATE_PATH = Path("data/logs/eham_metar_event_state.json")
METAR_PATH = Path("data/forecasts/awc_metar_live_EHAM.parquet")


def main() -> None:
    args = _parse_args()
    _activate_eham_telegram()
    refresh = refresh_awc_live("EHAM")
    ...
    if is_new:
        _run_forecast(args)
        _save_state(latest)
        print(json.dumps({"status": "new_metar_forecast", "airport": "EHAM", "latest_metar_time_utc": latest, "refresh": refresh}, indent=2))
        return
    print(
        json.dumps(
            {
...
def _run_forecast(args: argparse.Namespace) -> None:
    target_date = args.target_date or datetime.now(ZoneInfo("Europe/Amsterdam")).date().isoformat()
    command = [
        sys.executable,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Poll for a new EHAM METAR and notify forecast on each new report.")
