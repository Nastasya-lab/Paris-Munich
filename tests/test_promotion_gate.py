...
def test_shadow_promotion_gate_waits_for_enough_independent_outcomes():
    variants = pd.DataFrame(
        [
            ...
            _row("f1", "shadow_phase_arbitrated", 10.0, 1.5, 0.8, 0.2, 0.8, 8.0),
        ]
    )
...
def test_shadow_promotion_gate_allows_manual_review_when_quality_and_sample_pass():
    ...
    for idx in range(30):
        ...
        rows.append(_row(forecast_id, "shadow_phase_arbitrated", 20.0, 1.7, 0.8, 0.20, 0.82, hour, day=idx))
...
def test_shadow_promotion_gate_blocks_quality_failure_after_sample_passes():
    ...
    for idx in range(30):
        ...
        rows.append(_row(forecast_id, "shadow_phase_arbitrated", 20.0, 2.6, 1.4, 0.35, 0.65, hour, day=idx))
    gate = evaluate_shadow_promotion_gate(pd.DataFrame(rows))
...
def _row(
    ...
    *,
    ...
    return {
        ...
        "variant_version": PHASE_AWARE_SHADOW_VERSION if variant == "shadow_phase_arbitrated" else "production_dynamic_v1",
        "target_date_local": f"2026-07-{day % 28 + 1:02d}",
        "error_expected_c": -0.5 if variant == "shadow_phase_arbitrated" else 0.8,
        "nll": nll,
        "crps": crps,
